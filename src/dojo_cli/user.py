...
def change_settings():
    ...
    old_data = {
        ...
        if isinstance(name := element.get('name'), str) and name not in {'confirm', 'expiration'}
    }
    new_data = {}
...
def show_scoreboard(
    dojo_id: str | None = None, module_id: str | None = None, duration: str = 'all', page: int = 1, simple: bool = False
):
    if dojo_id:
