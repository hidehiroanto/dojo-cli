...
class CustomParameterControls(ParameterControls):
        ...
        with ControlGroupsContainer():
            ...
            if isinstance(argument_type, click.Choice) and multiple:
                ...
                multiple_choice_widget = control_method(
                    default=default, label=label, multiple=multiple, schema=schema, control_id=schema.key
                )
    ...
    def make_checkbox_control(
        default: MultiValueParamData, label: Text | None, multiple: bool, schema: OptionSchema | ArgumentSchema, control_id: str
        ...
        with NonFocusableVerticalScroll(classes='command-info-container'):
            ...
            with ContentSwitcher(initial='command-info-text', id='command-info-switcher'):
                ...
                yield CommandMetadata(
                    command_schema=self.command_schema, id='command-info-metadata', classes='command-info-metadata'
                )
...
def unwrap_hint(hint: Any) -> Any:
    ...
    if origin is Annotated:
        return unwrap_hint(get_args(hint)[0])
...
def build_click_argument(argument) -> click.Argument:
    ...
    kwargs: dict[str, Any] = {'required': argument.required, 'type': click_type_for(argument), 'default': default_for(argument)}
    if consume_all:
        kwargs['nargs'] = -1
...
def build_click_proxy(app: CycloptsApp, command_name: str | None = None) -> click.Command | click.Group:
    ...
    return click.Command(name=command_name, callback=build_click_callback(app), help=help_text, params=params)
