...
def run_editor(
    editor_name: str, path: Path | None = None, mount_point: Path | None = None
):
    cli = (
        str(SUPPORTED_EDITORS[editor_name]['cli'])
...
def init_editor(
    ...
    if UNAME_SYSTEM == 'Darwin':
        warn(
            f'You may see a popup like: [b yellow]{editor_name}.app would like '
            'to access files on a network volume.[/]'
        )
        warn('If so, please click [b green]Allow[/].')
        warn(
            'Otherwise, you may need to enable Full Disk Access so that the '
            'editor can access the mounted volume.'
        )
        warn(
            'If so, navigate to System Settings > Privacy & Security > Full '
            'Disk Access.'
        )
